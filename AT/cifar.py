...
parser.add_argument(
    "--save", default="", type=str, help="save parameters and logs in this folder"
)
parser.add_argument(
    "--ngpu", default=1, type=int, help="number of GPUs to use for training"
)
parser.add_argument(
...
def create_dataset(opt, train):
    ...
    return getattr(datasets, opt.dataset)(
        opt.dataroot, train=train, download=True, transform=transform
    )
...
def resnet(depth, width, num_classes):
    ...
    def gen_block_params(ni, no):
        return {
            "conv0": model.conv_params(ni, no, 3),
            "conv1": model.conv_params(no, no, 3),
            "bn0": model.bnparams(ni),
            "bn1": model.bnparams(no),
            "convdim": model.conv_params(ni, no, 1) if ni != no else None,
        }
    ...
    flat_params = model.cast(
        model.flatten({
            "conv0": model.conv_params(3, 16, 3),
            "group0": gen_group_params(16, widths[0], n),
            "group1": gen_group_params(widths[0], widths[1], n),
            "group2": gen_group_params(widths[1], widths[2], n),
            "bn": model.bnparams(widths[2]),
            "fc": model.linear_params(widths[2], num_classes),
        })
    )

    model.set_requires_grad_except_bn_(flat_params)

    def block(x, params, base, mode, stride):
        o1 = F.relu(model.batch_norm(x, params, base + ".bn0", mode), inplace=True)
        y = F.conv2d(o1, params[base + ".conv0"], stride=stride, padding=1)
        o2 = F.relu(model.batch_norm(y, params, base + ".bn1", mode), inplace=True)
    ...
    def f(input, params, mode, base=""):
        ...
        o = F.relu(model.batch_norm(g2, params, f"{base}bn", mode))
...
def main():
    ...
    # deal with teacher
    if opt.teacher_id:
        with open(os.path.join("logs", opt.teacher_id, "log.txt")) as ff:
            line = ff.readline()
        ...
        def f(inputs, params, mode):
            ...
            return y_s, y_t, list(starmap(model.at_loss, zip(g_s, g_t, strict=False)))
    ...
    def create_optimizer(opt, lr):
        ...
        return SGD(
            (v for v in params.values() if v.requires_grad),
            lr,
            momentum=0.9,
    ...
    print("\nParameters:")
    model.print_tensor_dict(params)
    ...
    def h(sample):
        inputs = model.cast(sample[0], opt.dtype).detach()
        targets = model.cast(sample[1], "long")
        if opt.teacher_id != "":
            y_s, y_t, loss_groups = model.data_parallel(
                f, inputs, params, sample[2], range(opt.ngpu)
            )
            loss_groups = [v.sum() for v in loss_groups]
            [m.add(v.item()) for m, v in zip(meters_at, loss_groups, strict=False)]
            return model.distillation(
                y_s, y_t, targets, opt.temperature, opt.alpha
            ) + opt.beta * sum(loss_groups), y_s
        y = model.data_parallel(f, inputs, params, sample[2], range(opt.ngpu))[0]
