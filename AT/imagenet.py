...
def get_iterator(imagenetpath, batch_size, nthread, mode):
    ...
    return DataLoader(
        ds, batch_size=batch_size, shuffle=mode, num_workers=nthread, pin_memory=True
    )
...
def define_teacher(params_file):
    ...
    blocks = [
        sum([
            re.match(r"group%d.block\d+.conv0.weight" % j, k) is not None
...
def define_student(depth, width):
    ...
    flat_params = OrderedDict(
        model.flatten({
            "conv0": model.conv_params(3, 64, 7),
            "bn0": model.bnparams(64),
            ...
            "fc": model.linear_params(widths[3], 1000),
        })
    )
    ...
    def block(x, params, base, mode, stride):
        ...
        o1 = F.relu(model.batch_norm(y, params, base + ".bn0", mode), inplace=True)
        z = F.conv2d(o1, params[base + ".conv1"], stride=1, padding=1)
        o2 = model.batch_norm(z, params, base + ".bn1", mode)
        if base + ".convdim" in params:
            return F.relu(
                o2 + F.conv2d(x, params[base + ".convdim"], stride=stride), inplace=True
            )
    ...
    def group(o, params, base, mode, stride, n):
        for i in range(n):
            o = block(
                o, params, "%s.block%d" % (base, i), mode, stride if i == 0 else 1
            )
        return o

    def f(input, params, mode, pr=""):
        o = F.conv2d(input, params[pr + "conv0"], stride=2, padding=3)
        o = F.relu(model.batch_norm(o, params, pr + "bn0", mode), inplace=True)
        o = F.max_pool2d(o, 3, 2, 1)
...
def main():
    ...
    params = OrderedDict(
        (k, p.cuda().detach().requires_grad_(p.requires_grad))
    ...
    def h(sample):
        ...
        y_s, y_t, loss_groups = model.data_parallel(
            f, inputs, params, mode, range(opt.ngpu)
        )
