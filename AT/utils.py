...
from torch.nn.parallel import gather, parallel_apply, scatter
...
def data_parallel(f, input, params, mode, device_ids, output_device=None):
    ...
    params_replicas = [
        {k: params_all[i + j * len(params)] for i, k in enumerate(params.keys())}
...
def flatten(params):
    return {
        ".".join(k): v for k, v in nested_dict(params).items_flat() if v is not None
    }
