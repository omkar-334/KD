...
def parse_args():
    ...
    parser.add_argument(
        "--data-dir",
        default="/home/xxx/cifar100",
        type=str,
        help="the diretory to save cifar100 dataset",
    )
    parser.add_argument(
        "arch", metavar="ARCH", default="multi_resnet50_kd", help="model architecture"
    )
    parser.add_argument(
        "--dataset",
        "-d",
    ...
    parser.add_argument(
        "--batch-size", default=128, type=int, help="mini-batch size (default: 128)"
    )
    parser.add_argument("--lr", default=0.1, type=float, help="initial learning rate")
    parser.add_argument("--momentum", default=0.9, type=float, help="momentum")
    parser.add_argument(
        "--weight-decay", default=5e-4, type=float, help="weight decay (default: 1e-4)"
    )
    parser.add_argument(
        "--print-freq", default=100, type=int, help="print frequency (default: 10)"
    )
    parser.add_argument(
        "--resume",
        default="",
    ...
    # kd parameter
    parser.add_argument(
        "--temperature", default=3, type=int, help="temperature to smooth the logits"
    )
    parser.add_argument("--alpha", default=0.1, type=float, help="weight of kd loss")
    parser.add_argument(
        "--beta", default=1e-6, type=float, help="weight of feature loss"
    )
...
def main():
    ...
    if args.cmd == "train":
        logging.info(f"start training {args.arch}")
        run_training(args)

    elif args.cmd == "test":
        logging.info(
            f"start evaluating {args.arch} with checkpoints from {args.resume}"
        )
        run_test(args)


def run_test(args, test_loader=None):
...
def run_training(args, train_loader=None, test_loader=None):
    writer = SummaryWriter(args.summary_folder)
    if args.dataset == "cifar100":
        model = models.__dict__[args.arch](num_classes=100)
    else:
    ...
    for current_epoch in range(args.start_epoch, args.epoch):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            loss1by4 = kd_loss_function(middle_output1, temp4.detach(), args) * (
                args.temperature**2
            )
            losses1_kd.update(loss1by4, input.size(0))

            loss2by4 = kd_loss_function(middle_output2, temp4.detach(), args) * (
                args.temperature**2
            )
            losses2_kd.update(loss2by4, input.size(0))

            loss3by4 = kd_loss_function(middle_output3, temp4.detach(), args) * (
                args.temperature**2
            )
            ...
            total_loss = (
                (1 - args.alpha) * (loss + middle1_loss + middle2_loss + middle3_loss)
            ...
            if i % args.print_freq == 0:
                ...
                logging.info(
                    ...
                    f"Loss {total_losses.val:.3f} ({total_losses.avg:.3f})\t"
        ...
        checkpoint_path = os.path.join(
            args.save_path, f"checkpoint_{current_epoch:05d}.pth.tar"
        )
        save_checkpoint(
            {
                "epoch": current_epoch,
...
def validate(args, test_loader, model, criterion, writer=None, current_epoch=0):
    ...
    logging.info(
        ...
        f"Middle1@1 {middle1_top1.avg:.3f}\t"
        f"Middle2@1 {middle2_top1.avg:.3f}\t"
        f"Middle3@1 {middle3_top1.avg:.3f}\t"
    )
    ...
    if writer is not None:
        ...
        logging.info(f" * Prec@1 {top1.avg:.3f}")
...
def adjust_learning_rate(args, optimizer, epoch):
    ...
    logging.info("Epoch [%s] learning rate = %s", epoch, lr)
