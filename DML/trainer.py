...
class Trainer:
    ...
    def __init__(self, config, data_loader):
        ...
        self.best_valid_accs = [0.0] * self.model_num
        ...
        print(
            f"[*] Number of parameters of one model: {sum([p.data.nelement() for p in self.models[0].parameters()]):,}"
        )
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (images, labels) in enumerate(self.train_loader):
                ...
                pbar.set_description(
                    f"{toc - tic:.1f}s - model1_loss: {losses[0].avg:.3f} - model1_acc: {accs[0].avg:.3f}"
                )
    ...
    def test(self):
        ...
        print(
            f"[*] Test loss: {losses.avg:.3f}, top1_acc: {top1.avg:.3f}%, top5_acc: {top5.avg:.3f}%"
        )
